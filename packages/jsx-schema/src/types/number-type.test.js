  ...
  describe('validators', () => {
    ...
    describe('extended cast', () => {
      ...
      for (const primitive of [ true, false, '100', 'Infinity' ])
        it(`primitive ${primitive} become ${Number(primitive)}`, () => {
          expect(<number cast/>.validate(primitive))
            .to.be.equal(Number(primitive))
        })

      it('does not cast NaN', () => {
        expect(() => <number cast/>.validate(NaN))
    ...
    describe('clamp', () => {
      ...
      it('clamps a value between two values', () => {
        expect(<number clamp={[ 1, 100 ]} />.validate(110))
          .to.be.equal(100)

        expect(<number clamp={[ 1, 100 ]} />.validate(0))
          .to.be.equal(1)
      })
