...
describe('Type', () => {
  ...
  it('most basic type, used for \'any\'', () => {
    ...
    expect(whatever.schemaType.constructor).to.be.equal(Type)
  ...
  describe('validators', () => {
    ...
    describe('validate', () => {
      ...
      it('allows arbitrary an function to be used as a validator', () => {
        ...
        expect(foo.validate(4)).to.be.equal(4 * 4)
        expect(foo.validate('foo')).to.be.equal('foo')
      ...
      it('can also take an array of functions', () => {
        ...
        expect(foo.validate([ ...'dear' ]))
          .to.be.equal('raed')

        expect(() => foo.validate([ ...'rab' ]))
    ...
    describe('default', () => {
      ...
      it('provided function gets called', () => {
        ...
        expect(typeof rando.validate()).to.be.equal('number')
      })
    ...
    describe('range', () => {
      ...
      it('throws if value does not satisfy declared range', () => {
        ...
        expect(() => below5.validate(5))
          .to.throw('must be less than 5')
      })

      it('ignores null and undefined', () => {
        for (const nil of [ null, undefined ])
          expect(() => <any range={[0, 5]} />.validate(nil))
            .to.not.throw()
      })
      ...
      it('uses valueOf for non-numeric inputs', () => {
        ...
        expect(() => <any range={['<=', 10, 'Gotta be small.']} />.validate(ben))
          .to.throw('Gotta be small.')

        expect(<any range={[500, 10000]} />.validate(ben))
          .to.be.equal(ben)
      ...
      it('throws if value does not have a numeric valueOf', () => {
        ...
        expect(() => between1and10.validate('sup'))
    ...
    describe('length', () => {
      ...
      it('throws if value does not have a numeric legnth', () => {
        ...
        expect(() => book.validate(501))
          .to.throw('value does not have a numeric length')
        expect(book.validate(novel))
          .to.be.equal(novel)
        expect(() => book.validate(short))
          .to.throw('must have more than 500 pages')
      })

      it('ignores null and undefined', () => {
        for (const nil of [ null, undefined ])
          expect(<any length={[0, 10]} />.validate(nil))
            .to.be.equal(nil)
      })
