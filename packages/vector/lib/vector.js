...
class Vector {
  ...
  static lerp(from, to, delta = 0) {
    ...
    const x = (0, _math.lerp)(from.x, to.x, delta);
    const y = (0, _math.lerp)(from.y, to.y, delta);
  ...
  static distance(from, to) {
    ...
    return (0, _math.sqrt)(this.sqrDistance(from, to));
  }
  ...
  static toVector(input) {
    ...
    if (typeof input === 'string') input = input.replace(/\[|\sVector\]/g, '').split(',').map(n => parseFloat(n));

    // filters an array down to numbers, and a vector will be created
    // out of the first two
    if (input instanceof Array) return new Vector(...input.filter(v => isFinite(v)));
  ...
  ilerp(to = Vector.zero, delta = 0) {
    this.x = (0, _math.lerp)(this.x, to.x, delta);
    this.y = (0, _math.lerp)(this.y, to.y, delta);
    return this;
  }
  ...
  irotate(deg) {
    const rad = deg * _math.PI / 180;
    const c = (0, _math.cos)(rad);
    const s = (0, _math.sin)(rad);
  ...
  get angle() {
    return (0, _math.atan2)(this.y, this.x) * 180 / _math.PI;
  }

  get magnitude() {
    return (0, _math.sqrt)(this.sqrMagnitude);
  }
  ...
  equals(vector) {
    ...
    return vector instanceof Vector && equalOrNaN(this.x, vector.x) && equalOrNaN(this.y, vector.y);
  }
